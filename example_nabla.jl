...
N = 5
B = randn(5, 5)
A = transpose(B)B + UniformScaling(1e-6)
...
y = transpose(x) * (A * x) # Temporary bracketting because we don't support RowVectors yet.
...
# closure.
@unionise f(x::AbstractVector) = transpose(x) * (A * x)
