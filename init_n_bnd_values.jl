...
function initial_value_lin(x, lb_foo, rb_foo)
    ### consistency ###
    global a = (lb_foo(epsilon_t) - rb_foo(epsilon_t)) /
               (epsilon_x - I_ * epsilon_x)
...
function get_higher_resolution_init_vals(
    L,
    T,
    p_low,
    Q_low,
...
function get_init_p_Q_const_pressures(
    I_,
    M,
    lb_foo_p,
    rb_foo_p,
    ...
    init_value_Q = ones(I_, M) *
                   sign(init_value_p_2[1, 1] - init_value_p_2[I_, 1]) *
                   sqrt(abs(init_value_p_2[1, 1] - init_value_p_2[I_, 1]) /
                        (alpha_ * L_pipe)) # / alpha_ * epsilon_x
    #set_lb_Q!(init_value_Q, lb_foo_Q)
    #set_rb_Q!(init_value_Q, rb_foo_Q)
    return init_value_p_2, init_value_Q
end
...
function get_init_value_grid(e_dict_p_Q_, I_, M)
    ...
    for (i, (k, v)) in enumerate(e_dict_p_Q_)
        ...
        init_value_p_2[(1+(i-1)*I_):((i)*I_), :] = p_2_range *
                                                   reshape(ones(M), 1, M)
        #end
    end
